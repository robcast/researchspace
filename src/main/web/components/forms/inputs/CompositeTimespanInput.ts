 ...
 */
import { createElement, cloneElement, Props, ReactNode, ReactElement, Children } from 'react';
...
export interface CompositeTimespanState {
  timespanType?: string; //'day' | 'year' | 'range' | 'unspecified';
...
export class CompositeTimespanInput extends SingleValueInput<ComponentProps, CompositeTimespanState> {
  ...
  render() {
    ...
    childComponents = Children.map(this.props.children, (child: ReactElement) => {
...
function getSelectValue(field: FieldState) {
  const val = field.values.first();
  if (FieldValue.isAtomic(val)) {
    return val.label;
  }
  return null;
}
